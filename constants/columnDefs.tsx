...
import { Product } from '@/db/product-managements';
...
export const ORDERS_COLUMN_DEFS: ColumnsType<any> = [
  {
    title: '# order',
    ...
    render: (category) => {
      ...
      return (
        <div className="flex items-center gap-4">
          <div className={`badge-icon badge-icon--sm bg-${selectedCategory.color}`}>
            <i className={`${selectedCategory.icon} text-base`} />
          </div>
          <span className="label-text">{selectedCategory.label}</span>
        </div>
      );
    },
    responsive: ['lg'],
...
export const TRANSACTIONS_COLUMN_DEFS: ColumnsType<any> = [
  {
    title: 'Date & Time',
    dataIndex: 'timestamp',
          ...
          {record.seller ? (
            <div className="flex items-center gap-[18px]">
              <div className="img-wrapper w-[60px] h-[60px] flex items-center justify-center shrink-0">
                <Image className="max-w-[50px]" src={record.seller.logo} alt={record.seller.name} />
              </div>
...
export const SELLERS_COLUMN_DEFS: ColumnsType<any> = [
  {
    title: 'Seller',
    dataIndex: 'seller',
    render: (text, record) => (
      <div className="flex gap-[26px]">
        <div className="img-wrapper flex items-center justify-center w-[63px] h-[63px] shrink-0">
          <Image className="max-w-[50px]" src={record.logo} alt={record.name} />
        </div>
...
export const PRODUCTS_COLUMN_DEFS: ColumnsType<Product> = [
  {
    title: (
  ...
  },
  {
    ...
    render: (_, record) => (
      <div className="flex items-center justify-end gap-11">
        <Link href={`${ADMIN_ROUTE}/products/${record.sku}`} aria-label="Edit">
          <i className="icon icon-pen-to-square-regular text-lg leading-none" />
        </Link>
